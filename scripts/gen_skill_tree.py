...
# (100 + level * 40 XP curve, ~2-5% per node, 4 rewards/10 nodes) - there's
# no live client in this sandbox to playtest actual combat balance, so
# keeping every category on the same mathematical shape and just varying
# *which* attributes it targets is the most defensible way to keep them
# comparably powerful without being able to verify it in play.
# ---------------------------------------------------------------------------
CATEGORIES.append(gen_category(
...
# (precision strikes), not raw power.
CATEGORIES.append(gen_category(
    ...
    ["attack_speed+4%", "move_speed+2%", "attack_speed+4%", "luck+1", "move_speed+2%",
     "attack_speed+4%", "melee_damage+2%", "luck+1", "attack_speed+4%", "move_speed+2%"],
))
...
CATEGORIES.append(gen_category(
    ...
    ["attack_damage+4%", "knockback+6%", "attack_damage+4%", "max_health+5%", "knockback+6%",
     "attack_damage+4%", "melee_damage+2%", "max_health+5%", "attack_damage+4%", "knockback+6%"],
))
...
# all-trades, small bonuses spread across both speed and power.
CATEGORIES.append(gen_category(
    ...
    ["attack_damage+2%", "attack_speed+2%", "melee_damage+3%", "move_speed+1%", "attack_damage+2%",
     "attack_speed+2%", "melee_damage+3%", "move_speed+1%", "attack_damage+2%", "attack_speed+2%"],
))
...
# reach/range and armor penetration over raw speed.
CATEGORIES.append(gen_category(
    ...
    ["reach+0.5", "attack_damage+3%", "knockback+4%", "reach+0.5", "melee_damage+2%",
     "attack_damage+3%", "reach+0.5", "knockback+4%", "melee_damage+2%", "attack_damage+3%"],
))

# Tachi - combo/quickdraw focused ("common" profile, moderate impact,
# multi-strike). Identity: speed + mobility + a bit of luck, similar
# flavor to daggers but leaning more damage than pure evasion.
CATEGORIES.append(gen_category(
    ...
    ["attack_speed+3%", "attack_damage+3%", "move_speed+1%", "attack_speed+3%", "luck+1",
     "attack_damage+3%", "attack_speed+3%", "move_speed+1%", "luck+1", "attack_damage+3%"],
))
...
# instead of vanilla's bow/crossbow ids).
# ---------------------------------------------------------------------------
CATEGORIES.append(gen_category(
