...
def main() -> None:
    ...
    ap.add_argument(
        "--set-latest",
        action="store_true",
        help="after appending the pinned release to the registry, also move the `latest` alias "
             "to it. Off by default: a mint APPENDS to the registry but does NOT auto-recommend "
             "the new build (a broken release must not silently become `latest`).",
    )
    ap.add_argument(
        "--set-latest-to",
        default=None,
        metavar="TAG",
        help="move the `latest` alias to an EXISTING registry tag and exit, with no download/hash "
             "pass (the fast, no-380MB way to change the recommended release). e.g. --set-latest-to v0.4.0",
    )
    args = ap.parse_args()

    if args.set_latest_to:
        if not RELEASE_JSON.exists():
            print(f"error: {RELEASE_JSON} does not exist yet", file=sys.stderr)
            sys.exit(1)
        reg = json.loads(RELEASE_JSON.read_text())
        if args.set_latest_to not in reg.get("releases", {}):
            print(f"error: tag {args.set_latest_to!r} is not in the registry "
                  f"(available: {', '.join(reg.get('releases', {}))})", file=sys.stderr)
            sys.exit(1)
        reg["latest"] = args.set_latest_to
        RELEASE_JSON.write_text(json.dumps(reg, indent=2) + "\n")
        print(f"moved `latest` -> {args.set_latest_to}. Next: git add nix/release.json && commit.")
        return
    ...
    if args.modrinth_only:
        ...
        entry = data["releases"][data["latest"]]
        modrinth = fetch_modrinth_server_file(entry["version"], entry["assetName"])
        if modrinth is None:
            sys.exit(1)
        entry["modrinth"] = {"projectId": MODRINTH_PROJECT_ID, **modrinth}
        RELEASE_JSON.write_text(json.dumps(data, indent=2) + "\n")
        print(f"wrote {RELEASE_JSON} (modrinth fields only, for {data['latest']})")
        return
    ...
    if neoforge_version is not None:
        entry["neoforgeVersion"] = neoforge_version
    if modrinth is not None:
        entry["modrinth"] = {"projectId": MODRINTH_PROJECT_ID, **modrinth}

    # Load the existing registry (or start one). Migrate the pre-registry
    # flat format (a single top-level tag/assetName/sha256...) into the new
    # releases{} map if we find it, so a one-off run upgrades the file too.
    registry = {"repo": REPO, "latest": tag, "releases": {}}
    if RELEASE_JSON.exists():
        existing = json.loads(RELEASE_JSON.read_text())
        if "releases" in existing:
            registry = existing
        elif existing.get("tag"):  # old flat format -> migrate that pin in
            registry["releases"][existing["tag"]] = {
                k: existing[k] for k in
                ("version", "assetName", "assetId", "assetApiUrl", "size",
                 "sha256", "sha256Hex", "neoforgeVersion", "modrinth")
                if k in existing
            }
    registry["repo"] = REPO
    registry.setdefault("releases", {})[tag] = entry
    # APPEND-only by default: a mint records the new release's hash but does
    # NOT auto-recommend it. Moving `latest` is a deliberate act (--set-latest
    # here, or `--set-latest-to <tag>` later) so a broken build never silently
    # becomes the recommended one.
    if args.set_latest or "latest" not in registry:
        registry["latest"] = tag
    registry["_comment"] = (
        "Registry of every published release's server-bundle asset. `releases` maps each "
        "release tag -> its GitHub asset (assetName) + pinned sha256 (SRI, what pkgs.fetchurl "
        "verifies) + sha256Hex (plain, for the module's warn-on-mismatch check) + size + "
        "neoforgeVersion. `latest` is an ALIAS naming whichever tag is the current RECOMMENDED "
        "release; nix/module.nix's `releaseTag` option defaults to \"latest\" and resolves "
        "through it. `repo` is the public GitHub owner/name the assets are fetched from. Every "
        "mint APPENDS its release here via scripts/update_nix_release.py; moving `latest` is a "
        "deliberate separate act (--set-latest / --set-latest-to <tag>). Do not hand-edit hashes "
        "except in emergencies; re-run the script to confirm."
    )
    # stable key order: _comment, repo, latest, releases
    ordered = {k: registry[k] for k in ("_comment", "repo", "latest", "releases") if k in registry}
    RELEASE_JSON.write_text(json.dumps(ordered, indent=2) + "\n")
    print(f"wrote {RELEASE_JSON}: appended {tag}"
          + (f", moved `latest` -> {tag}" if registry['latest'] == tag and args.set_latest else "")
          + f" (latest = {registry['latest']}, {len(registry['releases'])} releases)")
    if args.modrinth and modrinth is None:
        ...
        print("Next: git add nix/release.json && commit -- the flake now resolves this release.")
